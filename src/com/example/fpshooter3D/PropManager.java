...
public class PropManager {
    ...
    private void loadFile(String fnm) {
        ...
        try {
            scene = modelLoader.load("/media/mahedi/4ACA850ECA84F789/FALSE/WORKSPACE/JAVA/Killer Game Programming in Java_Andrew Davison/src/com/example/fpshooter3D/models/" + fnm);
        } catch (Exception e) {
            System.err.println(e);
            System.exit(1);
        }

        BranchGroup sceneBranchGroup = scene.getSceneGroup();

        TransformGroup objBoundsTG = new TransformGroup();
        objBoundsTG.addChild(sceneBranchGroup);

        String ext = getExtension(fnm);
        BoundingSphere objBoundingSphere = (BoundingSphere) sceneBranchGroup.getBounds();
        setBSPosn(objBoundsTG, objBoundingSphere.getRadius(), ext);
    ...
    private void getFileCoords(String fnm) {
        String coordFile = "/media/mahedi/4ACA850ECA84F789/FALSE/WORKSPACE/JAVA/Killer Game Programming in Java_Andrew Davison/src/com/example/fpshooter3D/models/" + getName(fnm) + "Coords.txt";
        try {
            ...
            while ((line = bufferedReader.readLine()) != null) {
                ...
                if (ch == 'p') {
                    ...
                    System.out.println(coordFile + ": did not recognise lne : " + line);
                }
            }
    ...
    private void doMove(Vector3d moveVector3d) {
        ...
        chgTransform3D.setTranslation(moveVector3d);

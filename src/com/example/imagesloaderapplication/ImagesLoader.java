...
public class ImagesLoader {
    ...
    private String getPrefix(String fnm) {
        ...
        if ((position = fnm.lastIndexOf(".")) == -1) {
            System.out.println("No prefix found for filename : " + fnm);
            return fnm;
        } else {
            return fnm.substring(0, position);
        }
    }
    ...
    public int loadNumImages(String fnm, int number) {
        String prefix = null;
        String postfix = null;
        int startPosition = fnm.lastIndexOf("*");

        if (startPosition == -1) {
            System.out.println("No '*' in filename : " + fnm);
            prefix = getPrefix(fnm);
        } else {
            prefix = fnm.substring(0, startPosition);
            postfix = fnm.substring(startPosition + 1);
        }
    ...
    public BufferedImage getImage(String name, int position) {
        ...
        if (position < 0) {
            ...
            return (BufferedImage) imsList.get(newPosition);
        }
        return (BufferedImage) imsList.get(position);
    }
    ...
    public BufferedImage getImage(String name, String fnmPrefix) {
        ...
        int position = getGroupPosition(name, fnmPrefix);
        if (position < 0) {
            return (BufferedImage) imsList.get(0);
        }
        return (BufferedImage) imsList.get(position);
    ...
    private BufferedImage loadImage2(String fnm) {
        ImageIcon imageIcon = new ImageIcon(getClass().getResource(IMAGE_DIR + fnm));
        if (imageIcon == null) {
            return null;
        }
        ...
        return makeBIM(image, width, height);
    }
    ...
    private Image readImage(String frm) {
        Image image = Toolkit.getDefaultToolkit().getImage(getClass().getResource(IMAGE_DIR + frm));
    ...
    public BufferedImage[] loadStripImageArray(String fnm, int number) {
        ...
        if ((stripImage = loadImage(fnm)) == null) {
            System.out.println("Returning null");
            return null;
        }

        int width = (int) stripImage.getWidth() / number;
        ...
        for (int i = 0; i < number; i++) {
            ...
            graphics2D.drawImage(stripImage, 0, 0, width, height, i * width, 0, (i * width) + width, height, null);
            graphics2D.dispose();
        }
        return strip;
    }

...
public class ImagesLoader {
    ...
    public BufferedImage loadImage(String fnm) {
        try {
            BufferedImage bufferedImage = ImageIO.read(getClass().getResource(IMAGE_DIR + fnm));
            int transparency = bufferedImage.getColorModel().getTransparency();
            BufferedImage copy = graphicsConfiguration.createCompatibleImage(bufferedImage.getWidth(),
    ...
    public BufferedImage[] loadStripImageArray(String fnm, int number) {
        ...
        int imageWidth = (int) stripImage.getWidth() / number;
        ...
        for (int i = 0; i < number; i++) {
            strip[i] = graphicsConfiguration.createCompatibleImage(imageWidth, height, transparency);
            graphics2D = strip[i].createGraphics();
            graphics2D.drawImage(stripImage, 0, 0, imageWidth, height, i * imageWidth, 0, (i * imageWidth) + imageWidth, height, null);
            graphics2D.dispose();
        }
        return strip;
    }
